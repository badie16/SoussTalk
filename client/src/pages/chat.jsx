  ...
  useEffect(() => {
    const loadUserAndContacts = async () => {
      ...
      try {
        ...
        // Check if we need to initiate a call (coming from calls page)
        if (location.state?.callContact) {
          const callContact = location.state.callContact
          const callType = location.state.callType || "audio"

          // Find the contact in our contacts list
          const contactToCall = userContacts.find((c) => c.name === callContact.name) || callContact

          // Initiate the call
          handleCallContact(contactToCall, callType)

          // Clear the location state
          window.history.replaceState({}, document.title)
        }

        // Check if we have a selected contact from navigation
        if (location.state?.selectedContact) {
          setSelectedChat(location.state.selectedContact)
          if (isMobileView) {
          ...
          <div className={`${isMobileView ? "w-full" : "flex-1"} overflow-hidden show-scrollbar-on-hover`}>
            {selectedChat ? (
              <ChatDetail
                ...
                onCallContact={() => handleCallContact(selectedChat, "audio")}
                onVideoCallContact={() => handleCallContact(selectedChat, "video")}
              />
            ) : (
